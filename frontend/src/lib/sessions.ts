...
export async function setParticipantVisibility(
  ...
  await updateDoc(
    ...
    { visibility, lastSeenAt: serverTimestamp() },
  );
}
...
export function subscribeToSongs(
  sessionId: string,
  onChange: (songs: SessionSong[]) => void,
  onError: (error: Error) => void,
): Unsubscribe {
  return onSnapshot(
    collection(db, "sessions", sessionId, "queue"),
    (snapshot) => {
      const songs = snapshot.docs
        .map((songDoc) => {
          ...
          return {
            ...
            status: data.status as SessionSong["status"],
...
export function subscribeToParticipantSongs(
  sessionId: string,
  participantId: string,
  onChange: (songs: SessionSong[]) => void,
  onError: (error: Error) => void,
...
export function getQueueSongs(songs: SessionSong[]) {
  ...
  for (const song of songs) {
    if (!song.played && !firstUnplayedByParticipant.has(song.participantId))
      firstUnplayedByParticipant.set(song.participantId, song);
  }
  return [...firstUnplayedByParticipant.values()].sort(
...
export function subscribeToHostSessions(
  hostId: string,
  onChange: (sessions: KaraokeSession[]) => void,
  onError: (error: Error) => void,
): Unsubscribe {
