...
function JoinRoomPage({ code, onBack }: { code: string; onBack: () => void }) {
  ...
  async function joinRoom(event: FormEvent) {
    ...
    try { await signInAnonymously(); const session = await findSessionByCode(code); if (!session) { setError('That room could not be found. Check the invite link and try again.'); return }; setRoomTitle(session.title); setJoined(true) }
    catch (joinError) { setError(joinError instanceof Error ? joinError.message : 'Unable to join this room.') }
    finally { setLoading(false) }
  }

  if (joined) return <ParticipantRoom title={roomTitle} code={code} nickname={nickname} />
...
function ParticipantRoom({ title, code, nickname }: { title: string; code: string; nickname: string }) {
