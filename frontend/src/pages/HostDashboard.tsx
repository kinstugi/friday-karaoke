...
function HostDashboard({
  ...
  onOpenSession: (title: string, code: string, sessionId: string) => void;
}) {
  ...
  return (
    ...
    >
      <Box className="dashboard-orb orb-one" />
      <Box className="dashboard-orb orb-two" />
      <Box
        component="header"
        sx={{
          position: "relative",
      ...
      >
        ...
        >
          <Box>
            ...
            <Typography color="text.secondary" sx={{ mt: 1 }}>
              Start a room and let your guests take over the queue.
            </Typography>
          </Box>
          <Button
        ...
        {error && (
          <Typography color="error.main" sx={{ mb: 3 }}>
            Could not load your sessions. Check your Firestore rules and try
            again.
          </Typography>
        )}
        {loading ? (
              ...
              {sessions.map((session, index) => (
                <Grid key={session.id} size={{ xs: 12, sm: 6, md: 4 }}>
                  <SessionCard
                    ...
                    songs={session.songsCount}
                    guests={session.guestsCount}
                    color={["#57317a", "#8b385e", "#315c69"][index % 3]}
                    roomCode={session.roomCode}
                    onView={() =>
                      onOpenSession(session.title, session.roomCode, session.id)
                    }
                  />
                </Grid>
              ))}
          ...
          >
            <Box>
              ...
              <Typography
                ...
                sx={{ mt: 0.5 }}
              >
                Every great karaoke night starts with one song.
              </Typography>
            </Box>
            <Button
              onClick={onCreateSession}
...
function EmptySessions({ onCreateSession }: { onCreateSession: () => void }) {
  return (
    <Box
      sx={{
        py: { xs: 6, md: 10 },
    ...
    >
      ...
      <Typography color="text.secondary" sx={{ mt: 1, mb: 3 }}>
        Create a session and invite your friends to sing along.
      </Typography>
      <Button
        onClick={onCreateSession}
        variant="contained"
