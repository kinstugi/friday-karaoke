...
function App() {
  ...
  const [page, setPage] = useState<
    "landing" | "auth" | "dashboard" | "create" | "room" | "join"
  >(joinCode ? "join" : "landing");
  const [room, setRoom] = useState({
    title: "Singalong Session",
    code: "SING-42",
    sessionId: "",
  });
  return (
    <ThemeProvider theme={theme}>
      <CssBaseline />
      {page === "landing" ? (
        <LandingPage onOpenAuth={() => setPage("auth")} />
      ) : page === "auth" ? (
        <AuthPage
          onBack={() => setPage("landing")}
          onAuthenticated={() => setPage("dashboard")}
        />
      ) : page === "create" ? (
        <CreateSessionPage
        ...
        <JoinRoomPage code={joinCode} onBack={() => setPage("landing")} />
      ) : (
        <HostDashboard
          onSignOut={() => setPage("landing")}
          onCreateSession={() => setPage("create")}
          onOpenSession={(title, code, sessionId) => {
            setRoom({ title, code, sessionId });
            setPage("room");
          }}
        />
      )}
